...
plot([time{1}(usBlocks),time{1}(usBlocks)]/60,[-20,20]*5,'k--')
plot([time{1}(usBlocks*2),time{1}(usBlocks*2)]/60,[-20,20]*5,'k--')
% plot([time{1}(usBlocks*3),time{1}(usBlocks*3)]/60,[-20,20],'k--')

uDelays = unique(newT.delay);
...
text(time{1}(usBlocks)/60,min(uDelays)+0.15*max(uDelays),['p=',num2str(p,2)]);
...
plot(time{1}(idxAfter)/60,p50{1}(idxAfter),'*','linewidth',2,'MarkerSize',8)
[~,p] = ttest2(blocksBefore,blocksAfter);
text(time{1}(2*usBlocks)/60,min(uDelays)+0.15*max(uDelays),['p=',num2str(p,2)]);

xlabel('time (minutes)')
ylabel('Point of equal Probability (ms)')
title('Point of Equal Probability')
axis([0,max(time{1})/60,min(uDelays),max(uDelays)])
makeFigureBig(h)
