function out = read_Intan_RHS2000_file_JK(filename, notch_filter_frequencies)

% read_Intan_RHS2000_file
%
% Version 1.0, 30 January 2017
%
%   Reworked by Jan Kubanek, March 2019, to return a struct of variables
%
...
frequency_parameters = struct( ...
    ...
    'notch_filter_frequencies', notch_filter_frequencies, ...
...
%             num_gaps);
    ...
    for nf = 1 : numel(notch_filter_frequencies)        
        ...
        for i=1:num_amplifier_channels
            amplifier_data(i,:) = ...
                notch_filter(amplifier_data(i,:), sample_rate, notch_filter_frequencies(nf), 10);
...
out.notes = notes;
out.frequency_parameters = frequency_parameters ;
out.stim_parameters = stim_parameters;
if (data_file_main_version_number > 1)
    out.reference_channel = reference_channel;
end

if (num_amplifier_channels > 0)
    out.amplifier_channels = amplifier_channels;
    if (data_present)
        out.amplifier_data = amplifier_data;
        if (dc_amp_data_saved ~= 0)
            out.dc_amplifier_data = dc_amplifier_data;
        end
        out.stim_data = stim_data;
        out.amp_settle_data = amp_settle_data;
        out.charge_recovery_data = charge_recovery_data;
        out.compliance_limit_data = compliance_limit_data;
        out.t = t;
    end
    out.spike_triggers = spike_triggers;
end
if (num_board_adc_channels > 0)
    out.board_adc_channels = board_adc_channels;
    if (data_present)
        out.board_adc_data = board_adc_data;
    end
end
if (num_board_dac_channels > 0)
    out.board_dac_channels = board_dac_channels;
    if (data_present)
        out.board_dac_data = board_dac_channels;
    end
end
if (num_board_dig_in_channels > 0)
    out.board_dig_in_channels = board_dig_in_channels;
    if (data_present)
        out.board_dig_in_data = board_dig_in_data;
    end
end
if (num_board_dig_out_channels > 0)
    out.board_dig_out_channels = board_dig_out_channels;
    if (data_present)
        out.board_dig_out_data = board_dig_out_channels;
    end
end
