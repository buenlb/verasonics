function [accuracy,day,slope,bias,downshift,scale,session,changeIdx,shortDelayIdx] = safety(tData,date,verbose)
...
if verbose
    ...
    % makeFigureBig(h);
    % h.Position = [680   732   639   246];
    if exist('shortDelayIdx','var') && 0
    ...
    %     text(20,81,['p=',num2str(p),',r=',num2str(lm.Rsquared.Ordinary)]);
        disp(['p=',num2str(p),',r=',num2str(lm.Rsquared.Ordinary)]);
    end
    
    xlabel('Days')
    ylabel('Accuracy (%)')
    minY = floor(min(accuracy*10))*10;
    axis([0,max(sNumber),minY,100])
    if ~isnan(changeIdx)
