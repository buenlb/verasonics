function [accuracy,slope,bias,downshift,scale,session,changeIdx,shortDelayIdx] = safetyDurable(tData,date)
...
for ii = 1:length(tData)
    ...
    result = tData(ii).result;%(tData(ii).Block<tData(ii).usBlock);
...
%     text(61,61,['p=',num2str(p2),',r=',num2str(lm2.Rsquared.Ordinary)]);
    ...
    x = coefCI(lm,1);
    x = x(:,1);
    yHat = x(1)+x(2)*sort(sNumber);
    plot(sort(sNumber),yHat,'k--','linewidth',2)
