...
function [Trans,TW] = doppler256_MR(delays,savePath,saveName)
...
% Process(1).classname = 'External';
% Process(1).method = 'setPulseLength';
% Process(1).Parameters = {'srcbuffer','receive',... % name of buffer to process.
% 'srcbufnum',1,...
% 'srcframenum',1,...
% 'dstbuffer','none'};
...
UI(1).Control = {'UserA1','Style','VsSlider',...
    'Label','Duration (s)',...
    'SliderMinMaxVal',[1,60,1],...
...
%CB#1
assignin('base','pulseLength',round(UIValue));
evalin('base', 'TW(1).Parameters(3)=pulseLength*Trans.frequency*1e6*2;');
